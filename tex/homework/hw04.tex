...
\author{}
\date{\vspace{-1.0em}\textbf{Due: Tuesday 10, 2026 (11:59 pm)}}
...
Use an FTCS discretization of the heat equation on the uniform grid:
\begin{equation}
T_{i,j}^{n+1}
=
T_{i,j}^{n}
...
Submit a Jupyter notebook (\texttt{.ipynb}) that reproduces all results.

\begin{enumerate}
  \item \textbf{Grid and time step.}
  Report $\Delta x$, $\Delta y$, your chosen $\Delta t$, and verify that your chosen grid spacing and time step satisfy the stability condition.
  \item \textbf{Contour plots.}
  Produce contour plots of $T(x,y,t)$ at the following times:
  \[
  ...
  Compute and plot
  \[
   T_{\max}(t) = \max \left\{ T(x,y,t) \right\}
  \]
  versus time.
